  ...
  try {
    ...
    if (m1) {
      ...
      const response = await axios.post(
        "http://52.90.212.140:3000/group/sendmessage",
        obj,
        {
          headers: {
            Authorization: token,
...
const fetchdata = async () => {
  ...
  try {
    const response = await axios.get("http://52.90.212.140:3000/group/gname", {
      headers: {
        authorization: token,
      },
    });
    ...
    if (array) {
      ...
      try {
        const response = await axios.get(
          `http://52.90.212.140:3000/group/messages?lastmsgId=${id}`,
          {
            headers: {
              Authorization: token,
            },
...
console.log(id.value)
    ...
    ul.innerHTML = "";
    const response = await axios.get(`http://52.90.212.140:3000/group/gmessage?id=${id.value}`);
